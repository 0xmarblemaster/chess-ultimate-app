...
interface VerifiedProps {
  ...
  /** The caller's own legion standing (members stripped); null if unmapped/no season. */
  legionStanding?: Omit<LegionStanding, 'members'> | null;
  /** The caller's Top-N proximity inside their legion; null if not scored. */
  topNProximity?: StudentProximity | null;
  /** Season Top-N size, for the proximity copy. */
  topN?: number;
}
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        {/* Legion standing widget + personal Top-N proximity (§9.2) */}
        {legionStanding && (
          <section
            data-testid="empire-legion-standing"
            className="rounded-2xl bg-white border border-slate-200 shadow-sm p-5"
          >
            <div className="flex items-center gap-3">
              {legionStanding.legion.crest_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={legionStanding.legion.crest_url}
                  alt=""
                  className="w-12 h-12 shrink-0 object-contain"
                />
              ) : (
                <div className="w-12 h-12 rounded-lg bg-slate-100 grid place-items-center text-2xl shrink-0">
                  {legionStanding.legion.totem || '🛡️'}
                </div>
              )}
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-slate-800 truncate">
                  {legionStanding.legion.name}
                </div>
                <div className="text-xs text-slate-500 mt-0.5">
                  {legionStanding.place} {tg('legion.place')} · {legionStanding.points}{' '}
                  {tg('legion.points')}
                </div>
              </div>
              <div className="text-right shrink-0">
                {legionStanding.place === 1 ? (
                  <span className="text-lg" aria-hidden="true">
                    👑
                  </span>
                ) : (
                  <span className="text-xs text-slate-500">
                    {tg('legion.gapToNext', { points: legionStanding.gap_to_above })}
                  </span>
                )}
              </div>
            </div>
            {topNProximity && topNProximity.legion_id && (
              <div
                data-testid="empire-topn-proximity"
                className="mt-3 pt-3 border-t border-slate-100 text-xs font-medium text-slate-600"
              >
                {topNProximity.in_top_n
                  ? tg('legion.inTopN', { n: topN })
                  : tg('legion.toTopN', { n: topN, points: topNProximity.points_to_top_n })}
              </div>
            )}
          </section>
        )}
