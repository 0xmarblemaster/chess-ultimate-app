/**
 * Per-org gamification sync runner (PRD §5.3).
 *
 * Extracted from the CRON route so it can be invoked two ways:
 *  - the CRON_SECRET-guarded `POST /api/chess-empire/gamification/sync` route
 *    (all orgs, scheduled), and
 *  - the admin Ops "Sync now" button (a single org, admin/owner-gated).
 *
 * One pass per org: pull new CE tournament_results from the cursor forward, award
 * XP+coins idempotently, replay streak bonuses, reverse deleted results, and
 * recompute the player_gamification read model. Zero-start (D-5) — first run pins
 * the cursor to "now" so historical results are never awarded.
 */
import 'server-only';
...
/** Run a single org's full sync pass, updating gamification_sync_state. */
export async function syncOrg(orgId: string): Promise<OrgSyncSummary> {
