#!/usr/bin/env python3
"""
Load Russian Education Data to Knowledge Database - Final Fixed Version
Properly handles multiline OpenAI API key from .env file
"""
...
def load_env_manually():
    """Manually load and fix multiline API key from .env file"""
    ...
    if env_path.exists():
        ...
        for line in lines:
            ...
            if line.startswith('OPENAI_API_KEY='):
                ...
                openai_key = key_part
                in_openai_key = True
            elif in_openai_key and not line.startswith('#') and line and '=' not in line:
                # Continuation of API key
                openai_key += line
            elif in_openai_key and ('=' in line or line.startswith('#')):
                # End of API key
                break
        
        # Set the complete API key
        os.environ['OPENAI_API_KEY'] = openai_key
        print(f"✅ Manually loaded API key: {openai_key[:20]}... (length: {len(openai_key)})")
        return openai_key
...
def get_weaviate_client():
    ...
    # Load the API key manually to handle multiline issue
    openai_key = load_env_manually()
...
if __name__ == "__main__":
    print("🚀 Starting Russian Education Data Loading (Final Fixed Version)")
    print("=" * 60)
